 ...
 */
  ...
  if ( (rp = rdy_head[USER_Q]) != NIL_PROC) {
	if (isonep(rp)){
		rdy_tail[ONE_Q]->p_nextready = rdy_head[ONE_Q];
		rdy_tail[ONE_Q] = rdy_head[ONE_Q];
		rdy_head[ONE_Q] = rdy_head[ONE_Q]->p_nextready;
		rdy_tail[ONE_Q]->p_nextready = NIL_PROC;

		rp = rdy_head[ONE_Q];
		rdy_head[USER_Q] = rdy_head[ONE_Q];
		rdy_tail[USER_Q] = rdy_tail[ONE_Q];
	}
	else if (istwop(rp)){
		if (rp->p_nextready != NIL_PROC){
			int max_pri = rp->act_pri;
			struct proc * next_proc = rp;
			struct proc * prev_next = rp;

			while (rp->p_nextready != NIL_PROC){
				if (rp->p_nextready->act_pri > max_pri){
					max_pri = rp->p_nextready->act_pri;
					next_proc = rp->p_nextready;
					prev_next = rp;
				}
				if (rp->p_nextready->act_pri + 1 <= MAX_AGE)
					rp->p_nextready->act_pri++;
				rp = rp->p_nextready;
			}
			next_proc->prev_act_pri = next_proc->act_pri;
			next_proc->act_pri = next_proc->base_pri;

			/* przepiac wskazniki */
			if (next_proc != rdy_head[TWO_Q]){
				if (next_proc == rdy_tail[TWO_Q])
					rdy_tail[TWO_Q] = prev_next;
				prev_next->p_nextready = next_proc->p_nextready;
				next_proc->p_nextready = rdy_head[TWO_Q];
				rdy_head[TWO_Q] = next_proc;
			}

			rp = rdy_head[TWO_Q];
			rdy_head[USER_Q] = rdy_head[TWO_Q];
			rdy_tail[USER_Q] = rdy_tail[TWO_Q];
		}
	}
	else if (isthreep(rp)){
		if (rp->p_nextready != NIL_PROC){
			int min_burst = rp->act_pri;
			struct proc * next_proc = rp;
			struct proc * prev_next = rp;

			while (rp->p_nextready != NIL_PROC){
				if (rp->p_nextready->act_pri < min_burst){
					min_burst = rp->p_nextready->act_pri;
					next_proc = rp->p_nextready;
					prev_next = rp;
				}
				rp = rp->p_nextready;
			...
			/* przepiac wskazniki */
			if (next_proc != rdy_head[THREE_Q]){
				if (next_proc == rdy_tail[THREE_Q])
					rdy_tail[THREE_Q] = prev_next;
				prev_next->p_nextready = next_proc->p_nextready;
				next_proc->p_nextready = rdy_head[THREE_Q];
				rdy_head[THREE_Q] = next_proc;
			}

			rp = rdy_head[THREE_Q];
			rdy_head[USER_Q] = rdy_head[THREE_Q];
			rdy_tail[USER_Q] = rdy_tail[THREE_Q];
		}
		rp->prev_act_pri = rp->act_pri;
		rp->act_pri--;
	}
	proc_ptr = rp;
	bill_ptr = rp;
 ...
 */
  ...
  if (isuserp(rp)){
	if (isonep(rp)){
		if (rdy_head[ONE_Q] == NIL_PROC){
			rdy_tail[ONE_Q] = rp;
		}
		rp->p_nextready = rdy_head[ONE_Q];
		rdy_head[ONE_Q] = rp;
	}
	else if (istwop(rp)){
		if (rdy_head[TWO_Q] == NIL_PROC){
			rdy_tail[TWO_Q] = rp;
		}
		rp->p_nextready = rdy_head[TWO_Q];
		rdy_head[TWO_Q] = rp;
	}
	else if (isthreep(rp)){
		if (rdy_head[THREE_Q] == NIL_PROC){
			rdy_tail[THREE_Q] = rp;
		}
		rp->p_nextready = rdy_head[THREE_Q];
		rdy_head[THREE_Q] = rp;
	}
	...
	else if (rdy_head[TWO_Q] != NIL_PROC){
		rdy_head[USER_Q] = rdy_head[TWO_Q];
		rdy_tail[USER_Q] = rdy_tail[TWO_Q];
	}
	else if (rdy_head[THREE_Q] != NIL_PROC){
		rdy_head[USER_Q] = rdy_head[THREE_Q];
		rdy_tail[USER_Q] = rdy_tail[THREE_Q];
	}
  }
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	if ( (xp = rdy_head[ONE_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[ONE_Q] = xp->p_nextready;
	...
	qtail = &rdy_tail[ONE_Q];
  }
  else if (istwop(rp)){
	if ( (xp = rdy_head[TWO_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[TWO_Q] = xp->p_nextready;
	...
	qtail = &rdy_tail[TWO_Q];
  }
  else if (isthreep(rp)){
	if ( (xp = rdy_head[THREE_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[THREE_Q] = xp->p_nextready;
  ...
  while (xp->p_nextready != rp)
	if ( (xp = xp->p_nextready) == NIL_PROC) break;
  if (xp != NIL_PROC){
	  xp->p_nextready = xp->p_nextready->p_nextready;
	  if (*qtail == rp) *qtail = xp;
  }

  /* Uaktualnij wskazania na kolejki */
  if (rdy_head[ONE_Q] != NIL_PROC){
	rdy_tail[USER_Q] = rdy_tail[ONE_Q];
  }
  else if (rdy_head[TWO_Q] != NIL_PROC){
