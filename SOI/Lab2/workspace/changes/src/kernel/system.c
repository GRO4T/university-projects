...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->act_pri = MIN_PRI;
  rpc->base_pri = MIN_PRI;
  rpc->ptype = rpp->ptype; 
  rpc->prev_act_pri = MIN_PRI;
...
struct proc * find_user_proc_by_pid(int pid){
	int i;
	for (i = 0; i < NR_TASKS + NR_PROCS; i++){
		if (proc[i].p_pid == pid && proc[i].p_priority != PPRI_NONE)
			return &proc[i];
	}
		return 0;
}
/* <modified>
				do_getpri
	Zwraca wartość odpowiedniego priorytetu (głównego lub priorytetu pomocniczego).
	Przyjmuje pid procesu oraz typ priorytetu jako int.
...
{
	int pid = m_ptr->m1_i1;
	int to_get = m_ptr->m1_i2;

	struct proc * p = find_user_proc_by_pid(pid);
		if (p != NIL_PROC){
			if (to_get == 0){
				return p->base_pri; 
			}
				...
				return p->prev_act_pri;
		}
	return -1;	
}
/* 				do_setpri
	Ustawia wartość odpowiedniego priorytetu.
	pid - m1i1
	ppri_type - m1i2
	value - m1i3
*/
...
{
	int pid = m_ptr->m1_i1;
	int to_set = m_ptr->m1_i2;
	int value = m_ptr->m1_i3;

	struct proc * p = find_user_proc_by_pid(pid);

	assert(to_set >= 0 && to_set <= 3);
	if (p->ptype == 2 && (to_set == 1 || to_set == 0))
		assert(value >= MIN_PRI && value <= MAX_AGE);

	if (p != NIL_PROC){
		struct proc * xp;
		int qtype;
		if (isonep(p)) qtype = ONE_Q;
		else if (istwop(p)) qtype = TWO_Q;
		else if (isthreep(p)) qtype = THREE_Q;

		xp = rdy_head[qtype];
		while (xp != NIL_PROC){
			if (xp == p){
				lock_unready(p);
				if (to_set == 0) p->base_pri = value;
				else if (to_set == 1) p->act_pri = value;
				else if (to_set == 2) p->ptype = value;
				lock_ready(p);
				return 0;
			}
			xp = xp->p_nextready;
