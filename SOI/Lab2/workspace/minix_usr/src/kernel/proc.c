...
/* <modified> */
  if (is1p(rp)){
	if (rdy_head[1_Q] != NIL_PROC)
		rdy_tail[1_Q]->p_nextready = rp;
	else
		rdy_head[1_Q] = rp;
	rdy_tail[1_Q] = rp;
	rp->p_nextready = NIL_PROC;
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	if ( (xp = rdy_head[1_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[1_Q] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
...
/* <modified> */
  if (rdy_head[1_Q] == NIL_PROC) return;
  else{
	  /* One or more ONE processes queued. */
	  rdy_tail[1_Q]->p_nextready = rdy_head[1_Q];
	  rdy_tail[1_Q] = rdy_head[1_Q];
	  rdy_head[1_Q] = rdy_head[1_Q]->p_nextready;
	  rdy_tail[1_Q]->p_nextready = NIL_PROC;
	  pick_proc();
	  return;
  }
}
